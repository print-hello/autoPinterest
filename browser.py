...
def create_firefox_extension():
    ...
    zip_file = os.path.abspath(os.path.dirname(__file__) + sep + "boot") + sep + "extension.xpi"
...
def set_selenium_local_session(
    ...
    geckodriver_path = os.path.join(os.path.abspath('.'), 'boot', 'geckodriver.exe')
...
def close_browser(browser, logger):
    ...
    try:
        p = subprocess.Popen(
            'cmd.exe /c' + r'taskkill /f /im geckodriver.exe',
